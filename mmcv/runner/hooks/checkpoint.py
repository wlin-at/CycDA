...
class CheckpointHook(Hook):
    ...
    def _save_checkpoint(self, runner):
        ...
        if runner.meta is not None:
            if self.by_epoch:
                cur_ckpt_filename = self.args.get(
                    'filename_tmpl', 'epoch_{}'+f'_{runner.timestamp}'+'.pth').format(runner.epoch + 1)
            else:
                cur_ckpt_filename = self.args.get(
                    'filename_tmpl', 'iter_{}'+f'_{runner.timestamp}'+'.pth').format(runner.iter + 1)
        ...
        if self.max_keep_ckpts > 0:
            if self.by_epoch:
                name = 'epoch_{}'+f'_{runner.timestamp}'+'.pth'
                current_ckpt = runner.epoch + 1
            else:
                name = 'iter_{}'+f'_{runner.timestamp}'+'.pth'
