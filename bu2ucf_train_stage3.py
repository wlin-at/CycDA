...
# from ..utils.utils_img_cls import train_model, initialize_model
from utils_img_model.utils_img_cls import get_env_id
from utils_img_model.MyImageFolder import load_mapping
from utils_img_model.train_img_cls import run


parser = argparse.ArgumentParser()
parser.add_argument('--target_train_vid_ps_label', required=True,  help='video-level pseudo labels for target training videos')

parser.add_argument('--ps_thresh_percent', default=1.0)
...
parser.add_argument('--val_frequency', default=30)
...
parser.add_argument('--model_name', default="resnet_contrast_tsn", choices=['resnet_grl_tsn', 'resnet_contrast_tsn'])
...
target_train_vid_level_pseudo_dict = args.target_train_vid_ps_label
...
ps_thresh_percent = args.ps_thresh_percent
...
run(source_dataset = source_dataset, target_dataset = target_dataset,
    ...
    ps_main_dir=ps_main_dir, ps_thresh_percent= ps_thresh_percent,
