...
from mmaction.utils import collect_env, register_module_hooks

from mmaction.utils.update_config import update_config
from mmaction.utils.utils import initialize_logger
from mmaction.models.model_da.model_da import model_analysis
...
def parse_args():
    ...
    # resume_from  loads both the model weights and optimizer status, and the epoch is also inherited from the specified checkpoint
    ...
    parser.add_argument(
        '--validate',
        action='store_true',
        help='whether to evaluate the checkpoint during training') # evaluate the model on the validation set
    parser.add_argument(
        '--test-last',
        action='store_true',
    ...
    parser.add_argument(
        ...
        default='none',
        help='job launcher')  # items for distributed job initialization launcher.  if set to None, it will test in a non-distributed mode
...
def main():
    ...
    if hasattr(cfg, 'DA_config'):
        # cfg.DA_config and cfg.work_dir are updated
    ...
    logger = initialize_logger(cfg, timestamp)
    ...
    if len(cfg.workflow) == 2:  #  val workflow is duplicated with   --validate
