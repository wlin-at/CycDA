...
from mmaction.utils.utils import save_vid_prediction, save_clip_prediction, save_features
from mmaction.utils.update_config import update_config
...
def inference_pytorch(args, cfg, distributed, data_loader):
    ...
    DA_config = cfg.DA_config if hasattr(cfg, 'DA_config') else None
    if not distributed:
        model = MMDataParallel(model, device_ids=[0])
        outputs = single_gpu_test(model, data_loader, DA_config=DA_config)
    else:
        ...
        outputs = multi_gpu_test(model, data_loader, args.tmpdir,
                                 args.gpu_collect, DA_config=DA_config)
...
def main():
    ...
    if rank == 0:
        if output_config.get('out', None):
            out = output_config['out']   # path of the output file
            # print(f'\nwriting results to {out}')
            # dataset.dump_results(outputs, **output_config)  #  outputs is a list of numpy array (n_class, )
            compute_features = False
            if hasattr(cfg.DA_config, 'compute_features'):
                if cfg.DA_config.compute_features:
                    compute_features = True
            if compute_features:
                vid_outputs = []
                vid_features = []
                for vid_output, vid_feat in outputs:
                    vid_outputs.append( np.squeeze(vid_output)) #  a list of (n_class, )
                    vid_features.append( vid_feat)  # a list of (1, feat_dim, )
                save_features(vid_outputs, vid_features, out, dataset, cfg)
            else:
                vid_outputs = outputs
                save_vid_prediction(vid_outputs, out, dataset, cfg=cfg)

            # if isinstance(outputs[0], tuple): #  outputs contain a vid-level prediction and clip-level predictions
            #     vid_outputs = []
            #     clip_outputs = []
            #     for vid_output, clip_output in outputs: # todo notice that here the outputs are NOT softmax scores
            #         vid_outputs.append( np.squeeze(vid_output) )
            #         clip_outputs.append(np.squeeze(clip_output))
            #     save_vid_prediction(vid_outputs, out, dataset, cfg= cfg)
            #     # save_clip_prediction(clip_outputs, out, dataset, cfg=cfg)
            #
            # else:
            #     vid_outputs = outputs
            #     save_vid_prediction(vid_outputs, out, dataset, cfg=cfg)
        ...
        if eval_config:
            eval_res = dataset.evaluate(vid_outputs, **eval_config)
