...
class RecognizerTempAgg(BaseRecognizer): # todo   a recognizer consists of a backbone + a classification head
    """ 3D Recognizer with the module of temporal aggregation

        in the config file, we only specify backbone, without classifiation
        and we customize our classification head here
    ...
    def __init__(self,
                 ...
                 clsfr_mlp_dim = None,
                 # vid_d_clsfr_mlp_dim=None,
                 # vid_clsfr_mlp_dim=None,
        ...
        super(RecognizerTempAgg, self).__init__(backbone, cls_head, neck, train_cfg, test_cfg)
    ...
    def forward_domain(self, imgs, domain_label = None, beta = None,   ):
        ...
        return pred_clip_cls, pred_clip_d, pred_vid_cls, pred_vid_d,
    ...
    def forward_train(self ,  data_batch,   **kwargs):
        ...
        if if_use_source_batch:
            ...
            pred_clip_cls_s, pred_clip_d_s, pred_vid_cls_s, pred_vid_d_s = \
                self.forward_domain(imgs_s, domain_label=0, beta=beta)
            label_clip_s = label_s.clone().repeat_interleave(n_clips)  # (batch_s * n_clips )
            label_s = label_s.unsqueeze(0) if label_s.shape == torch.Size([]) else label_s
            # clip-level classification loss  on source
            loss_cls_clip_s = torch.tensor(0) if weight_clip_clspred == 0 else ce(pred_clip_cls_s, label_clip_s)
        ...
        if if_use_target_batch :
            ...
            pred_clip_cls_t, pred_clip_d_t, pred_vid_cls_t, pred_vid_d_t = \
                self.forward_domain(imgs_t, domain_label=1, beta=beta)
            label_clip_t = label_t.clone().repeat_interleave(n_clips)  # (batch_t * n_clips )
            ...
            loss_cls_clip_t = ce(pred_clip_cls_t, label_clip_t)
            ...
            if w_pseudo:
                if weight_clip_clspred_vid_ps:
                    vid_ps_label_clip_t = ps_label_t.clone().repeat_interleave(n_clips)  # (batch_s * n_clips )
                ...
                loss_cls_clip_t_ps = torch.tensor(0) if weight_clip_clspred_vid_ps == 0 else ce(pred_clip_cls_t, vid_ps_label_clip_t)
        ...
        if if_use_source_batch:
            if weight_clip_clspred != 0:
                top1_acc_clip_s, _ = accuracy(pred_clip_cls_s.data, label_clip_s, topk=(1, 5))
                losses.update({
        ...
        if if_use_target_batch:
            ...
            top1_acc_clip_t, _ = accuracy(pred_clip_cls_t.data, label_clip_t, topk=(1, 5))
            losses.update({
