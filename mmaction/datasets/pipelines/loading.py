...
class SampleFrames:
    ...
    def __init__(self,
        ...
        self.keep_tail_frames = keep_tail_frames   #  whether to keep tail frames when sampling
        assert self.out_of_bound_opt in ['loop', 'repeat_last']

        self.random_sample_frames = random_sample_frames
        if self.random_sample_frames :
            # to randomly sample  [clip_len]  frames during training
            #  uniformly sample  [clip_len]  frames at test time
            # if self.test_mode:
            #     assert num_clips == 10
            # else:
            #     assert num_clips == 1
    ...
    def _get_train_clips(self, num_frames):
        ...
        """
        ori_clip_len = self.clip_len * self.frame_interval  # ori_clip_len = 16 * 2 = 32
        ...
        if self.keep_tail_frames:
            ...
            avg_interval = (num_frames - ori_clip_len + 1) // self.num_clips  # the average interval between two clips (the clip can only be sampled within the video,  last start position is   (num_frames - ori_clip_len +1)       )
            ...
            if avg_interval > 0:
                ...
                clip_offsets = np.sort(
                    np.random.randint(
                        num_frames - ori_clip_len + 1, size=self.num_clips))  # in the interval of (0, num_frames - ori_clip_len + 1), randomly choose 4 starting positions
            elif avg_interval == 0:
    ...
    def __call__(self, results):
        ...
        if self.random_sample_frames:
            if self.test_mode:
                # self.num_clips is 10
                frame_inds_list = []
                # uniformly sample  [self.clip_len]  frames  at test time
                frame_stride = int(total_frames // self.clip_len)

                if frame_stride == 0:   # in this case, total_frames < self.clip_len
                    #  choose 10 starting positions in  0 ~  (self.clip-len - total_frames)
                    start_pos_to_choose = list(range( self.clip_len - total_frames +1 ))
                    multi_ = int(  self.num_clips //  len(start_pos_to_choose) )
                    remainder = int( self.num_clips %  len(start_pos_to_choose) )
                    starting_pos_ =start_pos_to_choose* multi_ + random.sample( start_pos_to_choose, remainder )

                    for start_pos in starting_pos_:
                        # repeat first frame and last frame
                        frame_inds = np.array(  [0] * start_pos  + list(range( total_frames )) +  [total_frames -1] * ( self.clip_len - total_frames - start_pos)    )
                        frame_inds_list.append(  list(frame_inds))

                else:  # in this case, total_frames >= self.clip_len
                    # choose 10 starting positions in  0 ~  (total_frames - self.clip_len * frame_stride)
                    start_pos_to_choose = list(range( total_frames - self.clip_len * frame_stride  +1  ))
                    multi_ = int(self.num_clips // len(start_pos_to_choose))
                    remainder = int(self.num_clips % len(start_pos_to_choose))
                    starting_pos_ = start_pos_to_choose * multi_ + random.sample(start_pos_to_choose, remainder)
                    for start_pos in starting_pos_:
                        # starting_frame =  int( (total_frames - self.clip_len * frame_stride) // 2)
                        starting_frame = start_pos
                        stop_frame = int( starting_frame + (self.clip_len -1)*frame_stride +1 )
                        frame_inds = np.arange( starting_frame, stop_frame, frame_stride  )
                        frame_inds_list.append(list(frame_inds))
                frame_inds = np.array(frame_inds_list)


            else:
                #  self.num_clips is  1
                # randomly sample  [self.clip_len]  frames  at training time
                if total_frames >= self.clip_len:
                    frame_inds = np.array( random.sample( range( total_frames), self.clip_len ) ) # random sample without duplicates
                else:
                    # randomly choose a starting position, then repeat first frame and last frame
                    start_pos = random.sample( range( self.clip_len - total_frames +1 ), 1 )[0]
                    frame_inds = np.array(  [0] * start_pos  + list(range( total_frames )) +  [total_frames -1] * ( self.clip_len - total_frames - start_pos)    )
                    # frame_inds = np.array(  list(range( total_frames )) +  [total_frames-1 ] * ( self.clip_len - total_frames))
